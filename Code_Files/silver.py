"""
Silver Layer Staging Pipeline.
Processes and unifies streaming events and bulk historical backfill data into Delta Live Table `stg_rides`.
"""
...
# Create Silver Staging Streaming Table
dp.create_streaming_table("stg_rides")


# Bulk / Historical Data Backfill Flow
@dp.append_flow(target="stg_rides")
def rides_bulk():
    """Ingests historical bulk rides and casts timestamps."""
    df = spark.readStream.table("bulk_rides")
    return df.withColumn("booking_timestamp", col("booking_timestamp").cast("timestamp"))


# Real-time Stream Ingestion Flow
@dp.append_flow(target="stg_rides")
def rides_stream():
    """Parses streaming JSON payloads and flattens into staging table."""
